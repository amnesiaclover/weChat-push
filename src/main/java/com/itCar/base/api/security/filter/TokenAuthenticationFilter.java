...
//        logger.info("是否包含：[" + requestURI.contains("admin") + "]");
        ...
        try {
            authentication = this.getAuthentication(req);
        } catch (Exception e) {
            ResponseUtil.out(res, ResultBody.error("用户登录超时或未登录!请重新登录", 401));
        }
        ...
        if (authentication != null) {
            ...
            ResponseUtil.out(res, ResultBody.error("验证失败,请尝试重新登录", 401));
        }
