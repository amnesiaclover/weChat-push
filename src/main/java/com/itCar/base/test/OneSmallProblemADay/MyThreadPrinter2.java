 ...
 * @Description: TODO 建立三个线程，A线程打印10次A，B线程打印10次B,C线程打印10次C，要求线程同时运行，交替打印10次ABC。这个问题用Object的wait()，notify()就可以很方便的解决
 * @author: liuzg
 * @Date: 2022/6/21 9:41
 * @DAY_NAME_SHORT: 星期二
 * @Version: v1.0
...
public class MyThreadPrinter2 implements Runnable {
     ...
     * 就是前一个线程所持有的对象锁。还有一个就是自身对象锁。
     * 主要的思想就是，为了控制执行的顺序，必须要先持有prev锁，
     * 也就前一个线程要释放自身对象锁，再去申请自身对象锁，两者兼备时打印，
     ...
     * 线程B等待A锁，再申请B锁，后打印B，再释放B，A锁，唤醒C，线程C等待B锁，
     * 再申请C锁，后打印C，再释放C,B锁，唤醒A。看起来似乎没什么问题，但如果你仔细想一下，
     * 就会发现有问题，就是初始条件，三个线程按照A,B,C的顺序来启动，
